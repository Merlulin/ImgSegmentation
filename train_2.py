...
from models import fcn_resnet50, Unet
...
def create_model(num_classes: int , pretrain_model: bool=False, checkpoint=None):
...
def create_logger():
    ...
    # 输出到文件中的处理器
    fileHandler = logging.FileHandler(filename='Carvana_Unet.log', mode='w')
...
def main(args):
    ...
    model = create_model(num_classes=num_classes, pretrain_model=False)
    model.to(device)
    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        if (val_epoch_loss <= min_loss and dice_epoch_score >= max_score):
            ...
            torch.save(save_file, args.save_weight_path + "model_weight_Unet.pth")
